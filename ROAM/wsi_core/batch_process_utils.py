...
'''
def initialize_df(slides, slides_path, seg_params, filter_params, vis_params, patch_params, 
	...
	if isinstance(slides, pd.DataFrame):
		...
		slide_ids = slides
	default_df_dict = {'slide_id': slide_ids, 'slide_path':slides_path, 'process': np.full((total), 1, dtype=np.uint8)}
